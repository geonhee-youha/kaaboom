...
import { ArtistProps, FanProps } from "../types/user";
...
export function useNavigation(): UseSlideProps {
  ...
  const stackTo = useCallback((newValue: string, title?: string) => {
    ...
    setSlides((prev) => {
      ...
      slides = _.filter(slides, (el) => el.show === true);
      let slide: SlideProps = slides[slides.length - 1];
      let prevSlides: SlideProps[] = _.filter(
        slides,
        (el) => el.stack !== newValue
      );
      let currentSlides: SlideProps[] = _.filter(
        slides,
        (el) => el.stack === newValue
      );
      if (newValue === slide.stack) {
        ...
        slides = slides.map((el) => {
          return {
            ...el,
            show: el.name !== "home" && el.stack === newValue ? false : el.show,
          };
        });
        return slides;
      }
